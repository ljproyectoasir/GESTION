...
P_OUTPUT='DROP'
P_FORWARD='ACCEPT'
P_POSTROUTING='ACCEPT'
P_PREROUTING='ACCEPT'


### Servicios TCP que corren en el servidor (input) ###
TCP_IN=(${DNS} ${NRPE} ${NRPE})
TCP_IN_SOURCE=(${ALL} ${NAGIOS_1} ${NAGIOS_2})
TCP_IN_DEST=(${ALL} ${IP_LOCAL_1} ${IP_LOCAL_1})
